...
def preprocess_for_inference(path, tpl_base_gray, tpl_h, tpl_w, crop_config, rule_config):
    try:
        ...
        # [수정된 부분] (1) best_score 반환값 추가
        return final_tensor, rule_score, best_score
...
def main():
    ...
    def process_batch():
        ...
        for idx, pred_val in enumerate(preds):
            ...
            # [수정된 부분] (1) batch_info에서 best_score 받기
            filename, file_path, metadata, rule_score, best_score, ok_folder, esd_folder = batch_info[idx]
            ...
            row = {
                ...
                # [수정된 부분] (1) Matching_Score 컬럼 추가 (round(best_score, 2))
    ...
    for filename in tqdm(image_files, desc="Processing"):
        ...
        try:
            ...
            # [수정된 부분] (1) best_score 변수 반환받기
            tensor, rule_score, best_score = preprocess_for_inference(
            ...
            batch_info.append((filename, file_path, metadata, rule_score, best_score, OK_FOLDER_DYNAMIC, ESD_FOLDER_DYNAMIC))
    ...
    try:
        ...
        for gls_id, group_df in grouped:
            # [수정된 부분] (2) best_score (< 0.90) 비중에 따른 파일명 변경 로직
            total_rows = len(group_df)
            bad_match_rows = len(group_df[group_df["Matching_Score"] < 0.90])
            
            if total_rows > 0 and (bad_match_rows / total_rows) >= 0.05:
                output_excel_path = os.path.join(BASE_PATH, f"{gls_id}_focus_out.xlsx")
            else:
